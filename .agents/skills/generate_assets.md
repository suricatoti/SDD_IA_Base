...
1. Read the `Technical_Specification.md` and the JSON files inside `production_artifacts/ui_specs/`.
...
3. Update the existing JSON screen files inside `production_artifacts/ui_specs/` by injecting a new global key called `"design_tokens"` with these definitions.
...
4. If image generation tools are not active, document the exact image prompts and placeholder mappings inside a new file named `production_artifacts/ui_specs/visual_assets.md` so the user knows what to generate.