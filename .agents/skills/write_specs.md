...
Only AFTER the user approves the scope, translate the agreed requirements into a structured `Technical_Specification.md`. You MUST strictly follow this internal structure:
...
Save the final document inside the `production_artifacts/` directory so the Architect can design the system on top of these exact constraints.